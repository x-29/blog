...
如果发送方收到相同数据的 3 个冗余 ACK（之前已经收到过 1 个，现在又收到 3个），说明在这个已被确认的报文段之后可能有 3 个报文段已经丢失。因此，TCP 执行快速重传（fast retransmit），即重传最早未被确认的报文段，不用等到超时事件发生。
...
- TCP 让发送方维护一个接收窗口（receive window）变量。这个变量告诉发送方，接收方还有多少可用的缓存空间。TCP 是全双工通信，在连接两端的发送方都各自维护一个接收窗口。
...
数据交换完成后，连接可能就不需要了，这时，发送方和接收方的任何一方都能选择终止连接。
