...
假设在同一个子网中，IP 地址为 `222.222.222.220` 的源主机向目的主机 `222.222.222.222` 发送 IP 数据报。为了发送数据报，源主机不仅要向它的适配器提供 IP 数据报，而且还必须要提供目的主机的 MAC 地址。源主机怎么知道目的主机的 MAC 地址呢？这就需要 ARP 了。
- 发送主机中的 ARP 模块能够查询 IP 地址，返回相应的 MAC 地址。
- 每台主机或路由器在它们的内存中都维护了一张 ARP 表。这张表包含 IP 地址到 MAC 地址到映射关系。
- 如果 ARP 表中没有目的主机的表项，发送方就用 ARP 协议解析这个地址。
  - 首先，发送方构造一个称为 ARP 分组的特殊分组，并指示它的适配器用 MAC 广播地址（即 FF-FF-FF-FF-FF-FF）在局域网内发送这个分组。这个分组包含了发送和接收 IP 地址以及 MAC 地址字段。
  - 然后，适配器在链路层帧中封装这个 ARP 分组，用广播地址作为帧的目的地址，并将帧传输到子网中。
  - 子网中的每台主机和路由器的适配器都会收到这个帧，并且每个适配器都把在该帧中的 ARP 分组向上传递给 ARP 模块。ARP 模块检测它的 IP 地址是否与 ARP 分组中的目的 IP 地址相匹配。与之匹配的就给发送主机发送回一个带有 MAC 地址映射的响应 ARP 分组。
