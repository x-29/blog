...
- 子网部分。高 x 位，同一子网中的设备的子网部分相同。
- 主机部分。剩余的低位。

{{<figure src="/images/cidr-ip-addressing.jpg">}}

采用有点分十进制数形式 a.b.c.d/x 表示，其中 *x* 是地址中子网部分的位数。`/x`的记法也称为子网掩码（subnet mask）。一个地址剩余的 `32 - x` 位属于主机部分。

这样，多个子网就可以汇总（summarize）成一个因特网上的网络，例如，有 8 个组织（每个组织有自己的子网）通过同一个 ISP 连到因特网上，它们 IP 地址的高 21 位是相同的，只有低 3 位不同，这 8 个组织就可以汇总，在因特网上只需要一个路由表项，数据包通过因特网上的路由器到达 ISP，然后在 ISP 再通过次级的路由器选路到某个组织。