...
路由器基于目的地的转发可以描述为两个步骤：1）查找目的地 IP 地址（“匹配”）；2）然后将数据包发送到交换结构中的指定输出端口（“动作”）。这是一个 ”匹配+动作“ 的模式，把这种模式变得通用化会更有意义，通用转发不只是匹配目的地 IP 地址，还可以对协议栈的多个首部字段进行匹配，采取的动作也不只是转发，还可以是丢弃、复制、修改字段等。
