...
<!-- wp:paragraph -->
<p>每个页面的 PR 值都要经过多次计算后，才会趋向于稳定，也就是收敛的状态。一开始将所有页面的 PR 分值设为 1，带入上面的公式计算，每个页面都得到一个新的 PageRank 值。再把这些新的 PageRank 值继续带入上面的公式，得到更新的 PageRank 值，如此迭代计算，直到所有页面的PageRank 值几乎不再有大的变化才停止。</p>
<!-- /wp:paragraph -->